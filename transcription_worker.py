...
def transcribe_wav(source, destination):
    ...
    with wave.open(str(source), 'rb') as wav, open(destination, 'w', encoding='utf-8') as output:
        ...
        output.write('[')
        separator = ''
        for first, last, core, core_end in windows(total, rate):
            wav.setpos(first)
            ...
            segments, info = model.transcribe(audio, language=language, vad_filter=True,
                                             word_timestamps=True, beam_size=1 if low_memory() else 5,
...
if __name__ == '__main__':
    transcribe_wav(sys.argv[1], sys.argv[2])
